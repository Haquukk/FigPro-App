...
export const handlePaste = (
  ...
  if (clipboardData) {
    try {
      ...
      parsedObjects.forEach( async (objData: fabric.Object) => {
        const enlivenObjects =  await fabric.util.enlivenObjects([objData])
        
        // convert the plain javascript objects retrieved from localStorage into fabricjs objects (deserialization) 
        enlivenObjects.forEach((enlivenedObj) => {
          if (!(enlivenedObj instanceof fabric.Object)) {
            console.error("Invalid enlivened object. Skipping.");
            return;
          }
          enlivenedObj.set({
